document.addEventListener("DOMContentLoaded", async function () {
    ...
    function handleInputTextBlur() {
        // Agrega un pequeño retraso para que las sugerencias se puedan clicar antes de ocultarlas
        setTimeout(() => {
    ...
    async function getAutoCompleteSuggestions(query) {
        try {
            ...
            const apiUrl = `http://api.weatherapi.com/v1/search.json?key=${apiKey}&q=${encodeURIComponent(query)}`;
    ...
    function displayWeather(data) {
        ...
        const currentTempCElement = document.querySelector(".day-info .temp .cel span");
        const currentTempFElement = document.querySelector(".day-info .temp .feh span");
        ...
        cityNameElement.textContent = data.current?.location?.name || "San Francisco";
        currentDateElement.textContent = new Date().toLocaleDateString("en-US", { weekday: "long" });
        ...
        currentDescElement.textContent = data.current?.current?.condition?.text || "N/A";
    ...
    function displayFiveDayForecast(forecast) {
        ...
        for (let i = 0; i < forecast.length; i++) {
            ...
            if (dayForecast) {
                const dayOfWeek = new Date(dayForecast.date).toLocaleDateString("en-US", { weekday: "short" });
